...
public:
	...
	void SetPos(T x, U y) {
		m_vPos.x = static_cast<float>(x); m_vPos.y = static_cast<float>(y);
	}
	//객체의 위치값을 x값으로 고정시킨다.
	template<typename T>
	void SetPosX(T pos) {
	...
	void SetWorldPos(T x, U y) {
		m_vWorldPos.x = static_cast<float>(x); m_vWorldPos.y = static_cast<float>(y);
	}
	...
	void SetScale(T x, U y) {
		m_vScale.x = static_cast<float>(x); m_vScale.y = static_cast<float>(y);
	}

	template<typename T>
	//객체의 크기값을 scale값으로 고정시킨다.
	void SetScale(T scale) {
		m_vScale.x = static_cast<float>(scale); m_vScale.y = static_cast<float>(scale);
	}
	...
	void SetScalingCenter(T x, U y) {
		m_vScalingCenter.x = static_cast<float>(x); m_vScalingCenter.y = static_cast<float>(y);
	}
	...
	void SetRotCenter(T x, U y) {
		m_vRotCenter.x = static_cast<float>(x); m_vRotCenter.y = static_cast<float>(y);
	}
	...
	void SetRect(T left, U top, V right, W bottom) {
		m_Rect.left = static_cast<LONG>(left); m_Rect.top = static_cast<LONG>(top); m_Rect.right = static_cast<LONG>(right); m_Rect.bottom = static_cast<LONG>(bottom);
	}
